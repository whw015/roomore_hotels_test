...
try {
  ...
  // النزيل
  $st = $pdo->prepare("SELECT id, name_en, name_ar, email, phone FROM guests
  ...
  // العد
  $cst = $pdo->prepare("SELECT COUNT(*) FROM payments WHERE hotel_code=:hotel AND booking_id=:bid");
  $cst->execute([':hotel'=>$hotel, ':bid'=>$bk['id']]);
  $total = (int)$cst->fetchColumn();

  // الجلب
  $lim = (int)$per; $off=(int)$offset;
  $pst = $pdo->prepare("SELECT id, amount, currency, method, reference, status, notes, created_at
                        FROM payments
                        WHERE hotel_code=:hotel AND booking_id=:bid
                        ORDER BY created_at DESC
                        LIMIT $lim OFFSET $off");
  $pst->execute([':hotel'=>$hotel, ':bid'=>$bk['id']]);
  $rows = $pst->fetchAll();
  ...
  echo json_encode([
    ...
    'pagination'=>[
      'page'=>$page,'per_page'=>$per,'total'=>$total,'total_pages'=>(int)ceil($total/max(1,$per))
    ],
    'payments'=>$rows
